...
from .annotator.depth_anything import DepthAnythingV2
...
class Annotator:
    def __init__(self, name: str, device: str, joint_min_max: bool = False):
        if name == "hed":
            processor = HEDdetector()
        elif name == "depth":
            processor = DepthAnythingV2(joint_min_max)

        self.name = name
        self.processor = processor

    def __call__(self, images: list[Image.Image], width: int, height: int) -> list[np.ndarray]:
        imgs_np = [np.array(im) for im in images]

        if self.name == "depth":
            batch_result = self.processor(imgs_np)
            batch_result = [HWC3(cv2.resize(im, (width, height))) for im in batch_result]
        else:
            batch_result = []
            for img in imgs_np:
                result = self.processor(img)
                result = HWC3(cv2.resize(result, (width, height)))
...
configs = {
    ...
    "flux-dev-kontext": ModelSpec(
        ...
        repo_ae="ae.safetensors",
        ckpt_path=os.getenv("FLUX_KONTEXT"),
        params=FluxParams(
            in_channels=64,
            # out_channels=64,
            vec_in_dim=768,
